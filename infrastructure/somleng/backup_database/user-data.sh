...
then
  ...
  PGPASSWORD="${scfm_db_password}" pg_dump -Fc --host=${scfm_db_host} --username=${scfm_db_username} --dbname=${scfm_db_name} --port=5432 > "/tmp/$output_filename"
  aws s3 cp "/tmp/$output_filename" "s3://backups.somleng.org/db/$output_filename"
  aws s3 cp "s3://backups.somleng.org/db/$output_filename" "s3://backups.somleng.org/db/${scfm_db_name}_latest.dump"
fi
